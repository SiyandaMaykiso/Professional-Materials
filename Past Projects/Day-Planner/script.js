...
$(document).ready(() => {
        ...
        let timerUntilStartReloading = setInterval(function () {
            ...
            if (secondsElapsed === timeToRefresh) {
                console.log(moment());
                let isReloading = confirm("An hour has passed! Let's check for updates");
                if (isReloading) {
                    window.location.reload(true);
                } else {
                    alert("Automatic notifications will be turned off.");
                }
            }
        }, 1000);
    }); 
...
let timesArr= ["8AM","9AM", "10AM", "11PM", "12PM", "1PM", "2PM", "3PM", "4PM"]; 
...
function saveEvent(time,input){
    alert("This event has been saved"); 
...
function changeEvent(time, index, location, buttonEl,eventInput, isPopulated){
    if (eventInput.trim() === "" && isPopulated === "yes"){
        let isSaved= confirm("At "+time+": Would you like to remove the event '"+savedDayPlans[index].event+"' ?"); 
        clearEvent(isSaved,index,location, buttonEl); 
    } else if (eventInput.trim() !== "" && isPopulated ==="none"){
        let isSaved= confirm("At "+time+": Would you like to create the event '"+eventInput+ "'?"); 
        if(isSaved) {
        ...
        if (savedDayPlans[index].event !== eventInput){     
            let isSaved= confirm("At "+time+": Would you like to reschedule the event from '"+savedDayPlans[index].event+"' to '"+eventInput+"'?"); 
            if(isSaved) {
                removeEvent(index); 
                saveEvent(time, eventInput); 
            } else{
                alert("Change was not recorded."); 
                location.val(savedDayPlans[index].event); 
            }
        }
     }
...
$("#clear").on("click",function(){
    if(confirm("Are you sure you want to remove all saved events?")){
       clearLocalStorage(); 
...
// Save all- I feel like I should have made some of this into functions to avoid repetition  but its hard to wrap my mind around now after the fact 
 $("#saveAll").on("click", function(){   
    ...
    populateSavedEvents(); 
    alert("There are no unrecorded changes"); 
 }); 
